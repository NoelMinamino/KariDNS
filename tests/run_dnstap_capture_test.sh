...
perl "$SCRIPT_DIR/mock_dnstap_receiver.pl" --socket "$SOCK_PATH" --output "$LOG_PATH" --max-frames 4 --timeout 10 &
PID_RECEIVER=$!
...
# 6. Wait for receiver to finish capturing 4 frames (2 queries + 2 responses)
...
if grep -q "version=0.2.1" "$LOG_PATH"; then
