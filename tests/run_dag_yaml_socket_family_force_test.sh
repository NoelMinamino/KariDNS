...
# ==============================================================================
# KariDNS dag(1) YAML -4/-6 socket_family Priority & Verification Test Suite
# ==============================================================================
...
PORT4=$((21000 + $$ % 4000))
PORT6=$((26000 + $$ % 4000))
TMP_DIR="/tmp/karidns_yaml_sockfam_force_test_$$"
mkdir -p "$TMP_DIR"
...
echo "=== 1. Testing Explicit -4 +yaml Forces socket_family: INET ==="
echo -n "Test: -4 option reports socket_family: INET ... "
OUT_V4=$($DAG -4 @127.0.0.1 -p $PORT4 example.com A +yaml 2>&1 || true)
...
echo "=== 2. Testing Explicit -6 +yaml Forces socket_family: INET6 ==="
...
if perl "$TMP_DIR/mock_udp6.pl" "$PORT6" 2>/dev/null & SRV6_PID=$!; then
    sleep 0.3
    echo -n "Test: -6 option reports socket_family: INET6 ... "
    OUT_V6=$($DAG -6 @::1 -p $PORT6 example.com A +yaml 2>&1 || true)
...
echo "========================================================="
if [ "$FAILED" -eq 0 ]; then
    echo "🎉 ALL YAML SOCKET_FAMILY FORCE TESTS PASSED!"
    exit 0
else
    echo "❌ $FAILED YAML SOCKET_FAMILY FORCE TESTS FAILED!"
    exit 1
fi
