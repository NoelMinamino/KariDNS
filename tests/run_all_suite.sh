...
register_test "dag" "sh" "tests/run_dag_badcookie_transport_test.sh" "dag BADCOOKIE transport, +keepopen, +expire, +all modes"
register_test "dag" "sh" "tests/run_dag_bind_address_family_mismatch_test.sh" "dag -b bind address family mismatch detection"
register_test "dag" "sh" "tests/run_dag_break_help_examples_test.sh" "dag --break help examples syntax and execution test"
register_test "dag" "sh" "tests/run_dag_cli_options_test.sh" "dag CLI Options, IDN, EDNSOPT, Prereq, and Break Validation"
...
register_test "dag" "sh" "tests/run_dag_doh_dot_axfr_test.sh" "dag multi-message AXFR streaming & memory lifecycle over DoH/DoT"
register_test "dag" "sh" "tests/run_dag_ede_truncation_regression_test.sh" "dag EDE (0-29) and UDP TC fallback regression test"
register_test "dag" "sh" "tests/run_dag_edge_cases_audit_test.sh" "dag edge cases audit regression test suite"
register_test "dag" "sh" "tests/run_dag_edge_cases_phase3_test.sh" "dag edge cases phase 3 test suite"
register_test "dag" "sh" "tests/run_dag_fixes_validation_test.sh" "dag bug fixes & improvements comprehensive validation"
register_test "dag" "sh" "tests/run_dag_hex_payload_overflow_test.sh" "dag hex payload buffer overflow & sentinel validation"
register_test "dag" "sh" "tests/run_dag_ixfr_uptodate_test.sh" "dag IXFR up-to-date (single SOA) completion test"
register_test "dag" "sh" "tests/run_dag_keepopen_keepalive_test.sh" "dag +keepopen (RFC 7766) & +keepalive (RFC 7828) persistence"
register_test "dag" "sh" "tests/run_dag_keepopen_partial_read_test.sh" "dag +keepopen partial read TCP cache invalidation"
register_test "dag" "sh" "tests/run_dag_keepopen_tls_partial_read_test.sh" "dag +keepopen +tls partial read & desync transparency"
register_test "dag" "sh" "tests/run_dag_long_label_name_expansion_test.sh" "dag long label (63-byte) domain name expansion test"
register_test "dag" "sh" "tests/run_dag_multi_server_semantic_match_test.sh" "dag multi-server status match test suite"
register_test "dag" "sh" "tests/run_dag_multi_server_timeout_test.sh" "dag multi-server partial failure & summary output"
register_test "dag" "sh" "tests/run_dag_multiline_ds_single_line_test.sh" "dag +multiline DS single-line formatting"
register_test "dag" "sh" "tests/run_dag_replay_compare_recorded_test.sh" "dag replay --compare-recorded regression test"
register_test "dag" "sh" "tests/run_dag_replay_diff_test.sh" "dag replay --diff and diff-file validation"
register_test "dag" "sh" "tests/run_dag_rr_differential_test.sh" "dag RR wire differential test"
register_test "dag" "sh" "tests/run_dag_sig0_update_test.sh" "dag RFC 3007 / RFC 2931 SIG(0) client-side transaction updates"
register_test "dag" "sh" "tests/run_dag_tcp_connect_timeout_test.sh" "dag TCP/TLS connect timeout test (RFC 7858, RFC 8484)"
register_test "dag" "sh" "tests/run_dag_trace_cname_glue_test.sh" "dag +trace glue fallback and CNAME chain tracing"
...
register_test "dag" "sh" "tests/run_dag_update_del_no_type_test.sh" "dag --update-del type omission regression test"
register_test "dag" "sh" "tests/run_dag_yaml_apostrophe_escaping_test.sh" "dag YAML single-quote / apostrophe escaping"
register_test "dag" "sh" "tests/run_dag_yaml_cookie_status_test.sh" "dag YAML DNS cookie status verification"
register_test "dag" "sh" "tests/run_dag_yaml_ede_escaping_test.sh" "dag YAML EDE double-quote escaping"
register_test "dag" "sh" "tests/run_dag_yaml_edns_options_test.sh" "dag YAML output EDNS options parity validation"
register_test "dag" "sh" "tests/run_dag_yaml_expandaaaa_test.sh" "dag YAML AAAA standard notation compatibility"
register_test "dag" "sh" "tests/run_dag_yaml_no_spurious_resolve_error_test.sh" "dag YAML clean stream (no spurious resolution errors)"
register_test "dag" "sh" "tests/run_dag_yaml_nocrypto_test.sh" "dag YAML +nocrypto compatibility test"
register_test "dag" "sh" "tests/run_dag_yaml_rdata_test.sh" "dag YAML output RDATA & section validation"
