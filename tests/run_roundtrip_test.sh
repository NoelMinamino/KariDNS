...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo "[FAIL] Server failed to start. ASan output:"
    cat server_asan.log
    exit 1
fi
...
axfr_output=$(./dag-asan example.com AXFR @127.0.0.1 -p 10053 -y transfer-key:dGVzdC1vbmx5LWR1bW15LWtleS1kby1ub3QtdXNl +tcp)
