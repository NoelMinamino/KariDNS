...
#define LOG_ROTATION_MODE_SIZE 1
#define LOG_ROTATION_MODE_DATE 2
#define LOG_ROTATION_MODE LOG_ROTATION_MODE_SIZE
#define LOG_MAX_SIZE (100 * 1024 * 1024)

#define MAX_RECORDS 500000
#define ARENA_POOL_SIZE (1024 * 1024 * 64) // 64MB
...
static int write_dns_name_str(uint8_t *packet_buf, uint16_t *offset, const char *name, compress_ctx_t *ctx) {
    uint8_t wire[256];
    size_t w_len = 0;
    const char *p = name;
    while (*p) {
...
static int serialize_dns_record(uint8_t *res, size_t max_res_len, uint16_t *offset_ptr, dns_record_t *rec, compress_ctx_t *comp_ctx, const char *owner_name, uint32_t override_ttl) {
