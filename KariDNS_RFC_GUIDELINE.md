...
| RFC 9018 | Interoperable Domain Name System (DNS) Server Cookies | ✅ Full | `generate_server_cookie()` implements the interoperable 16-byte format (Version(1) + Reserved(3) + Timestamp(4) + Hash(8)) recommended by RFC 9018 (`dns_server_core.c`, ~line 3425) |
...
| RFC | Title | Status | Evidence / Notes |
|---|---|---|---|
| RFC 1035 | Base Record Types (A, NS, MD, MF, CNAME, SOA, MB, MG, MR, NULL, WKS, PTR, HINFO, MINFO, MX, TXT) | ✅ Full | Fully parsed, cached, and serialized in wire format (`dns_wire.c`, `dns_utils.c`, `dns_zone_parser.c`) |
...
Items marked ✅ Full for changes made during recent review rounds (RFC 8624, RFC 2181, RFC 8976/ZONEMD,
RFC 7766/9210, RFC 1912, RFC 5452, RFC 9619, RFC 9859, RFC 9824, RFC 10029, RFC 6066, RFC 7230,
Forward Zones) have been confirmed to compile cleanly and verified functionally:
- ZONEMD verification was independently re-run against the official RFC 8976 Appendix A.4 test vector
  and confirmed VALID.
- `karicheck` was rebuilt as a real binary and run against test zones in the repository.
- RFC 9619/9859/9824/10029/6066/7230 additions and `type "forward"` zone implementations were confirmed
  present and consistent with their documented evidence via direct source inspection and automated test suites
  (`tests/run_forward_zone_test.sh`, `tests/run_dag_ci_test.sh`, `tests/run_program_zone_test.sh`).
